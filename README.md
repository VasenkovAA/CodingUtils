...
- **`tui-manager`** — интерактивный TUI (терминальный интерфейс на [Textual](https://textual.textualize.io/)): модуль **Comment Cleaner** — сканирование комментариев по проекту, просмотр дерева файлов, сохранённые решения «что удалить», предпросмотр diff и применение изменений. Удобен, когда нужен пошаговый контроль вместо чисто CLI.
...
Для **`tui-manager`**: терминал с поддержкой современного TUI (Unicode, достаточный размер окна). Зависимости `textual` и `rich` ставятся вместе с пакетом.
...
### 4) TUI: интерактивная чистка комментариев
```bash
tui-manager
```
Откроется менеджер модулей; встроенный **Comment Cleaner** позволяет выбрать каталоги, отсканировать комментарии и применить удаление с предпросмотром.
